...
describe('SearchBar.vue', () => {
  ...
  it('disables audio checkbox and does not validate or emit it', async () => {
    wrapper.setData({ query: 'saturn', media_types: ['audio'] });
    await wrapper.find('button').trigger('click');
    
    // Should emit only 'image' since 'audio' is disabled
    expect(wrapper.emitted().search).toBeFalsy(); // No search should be emitted with just 'audio'
  });
});
